...
def compile_module(defines, datatype, dropout_probability, instance_id):
    with open("encoder_parameters.cuh", "w") as f:
        ...
        all_layouts += [
            ('WKKself', WK_K_self_layout),
            ('WQQself', WK_K_self_layout),
            ('WVVself', WK_K_self_layout),
            ('BK', BK_layout),
            ('BQ', BK_layout),
            ('BV', BK_layout),
            ('DBK', BK_layout),
            ...
            ('KK', WK_K_self_layout.translate({ord('J'): 'K'})),
            ('QQ', WK_K_self_layout),
            ('VV', WK_K_self_layout.translate({ord('J'): 'K'})),
            ('DKK', WK_K_self_layout.translate({ord('J'): 'K'})),
            ('DQQ', WK_K_self_layout),
            ('DVV', WK_K_self_layout.translate({ord('J'): 'K'})),
            ('KKself', WK_K_self_layout),
            ('QQself', WK_K_self_layout),
            ('VVself', WK_K_self_layout),
            ('DKKself', WK_K_self_layout),
            ('DQQself', WK_K_self_layout),
            ('DVVself', WK_K_self_layout),
            ('WK', WK_layout),
            ('WQ', WK_layout),
            ('WV', WK_layout),
            ('DWK', WK_layout),
    ...
    print('Compiling...')
    subprocess.run(f"nvcc -O3 -gencode arch=compute_61,code=sm_61 -gencode arch=compute_70,code=sm_70 -c --compiler-options -fPIC encoder.cu -o encoder.o".split(' '))
...
if __name__ == '__main__':
    ...
    numpy_params['X'] = np.einsum('BJN->BJN', X.clone().detach().cpu().numpy())
    ...
    KK = encoder.debug['KKQQVV'][0] # phbj
    QQ = encoder.debug['KKQQVV'][1]
    VV = encoder.debug['KKQQVV'][2]

    WK = encoder.debug['WKQV'][0] # phi
    WQ = encoder.debug['WKQV'][1]
    WV = encoder.debug['WKQV'][2]

    refWQ = numpy_params['WQ'] # hpi
    print('WQ: ', isclose(np.einsum('phi->hpi', WQ), refWQ))

    refWK = numpy_params['WK'] # hpi
    print('WK: ', isclose(np.einsum('phi->hpi', WK), refWK))

    refWV = numpy_params['WV'] # hpi
    print('WV: ', isclose(np.einsum('phi->hpi', WV), refWV))

    refQQ = np.einsum("bhjp->phbj", np.stack(i_attn_proj_q))
    print('QQ: ', isclose(QQ, refQQ))

    refKK = np.einsum("bhkp->phbk", i_attn_proj_k)
    print('KK: ', isclose(KK, refKK))

    refVV = np.einsum("bhkp->phbk", i_attn_proj_v)
    print('VV: ', isclose(VV, refVV))

    refALPHA = np.einsum("bhjk->hbjk", i_attn_scaled_scores)
    ALPHA = encoder.debug['ALPHA']
    print('ALPHA: ', isclose(ALPHA, refALPHA))

    myY = np.einsum('BJI->BJI', Y.detach().cpu().numpy())
    ...
    print('DX: ', isclose(DX, dx))
    
    DWK = encoder.debug['DWKQV'][0] # phi
    DWQ = encoder.debug['DWKQV'][1]
    DWV = encoder.debug['DWKQV'][2]
    print('DWK: ', isclose(np.einsum('phi->hpi', DWK), dattn_wk))
    print('DWQ: ', isclose(np.einsum('phi->hpi', DWQ), dattn_wq))
    print('DWV: ', isclose(np.einsum('phi->hpi', DWV), dattn_wv))

    DBKQV = encoder.debug['DBKQV']
    DBQKV = np.stack((DBKQV[1], DBKQV[0], DBKQV[2]))
    print('DBQKV: ', isclose(DBQKV, dattn_in_b))

    DWO = encoder.debug['DWO']
    refDWO = np.einsum("IHP->PHI", dattn_wo.reshape(I, H, P))
    ...
    print('DLINW1: ', isclose(encoder.debug['DLINW1'], dlinear1_w))

    print('DLINB2: ', isclose(encoder.debug['DLINB2'], dlinear2_b))

    print('DLINW2: ', isclose(encoder.debug['DLINW2'], dlinear2_w))
