"""Sweep and profile multi-head attention."""
...
for batch_size in batch_sizes:
    for seq_len in seq_lens:
        for embed_size in embed_sizes:
            q, k, v = gen_qkv(batch_size, seq_len, embed_size, args.attn_type)
            for num_heads in heads:
                ...
                try:
                    attn_times = time_multihead_attention(
                        q, num_heads, k=k, v=v, mask=args.mask, mode=args.attn_type,
                        do_backprop=True, fp=args.fp)
                    fwd_t = statistics.mean(attn_times[0])
                    fwd_stdev = statistics.stdev(attn_times[0])
                    bwd_t = statistics.mean(attn_times[1])
                    bwd_stdev = statistics.stdev(attn_times[1])
...
do_plot(batch_df.batch, batch_df, 'Mini-batch size',
        f'Batch time, seq len={args.base_seq_len} embed size={args.base_embed_size} heads={args.base_heads} attn={args.attn_type} mask={args.mask} fp={args.fp}',
...
do_plot(seq_df.seq_len, seq_df, 'Sequence length',
        f'Sequence length time, batch={args.base_batch_size} embed size={args.base_embed_size} heads={args.base_heads} attn={args.attn_type} mask={args.mask} fp={args.fp}',
...
do_plot(embed_df.embed, embed_df, 'Embedding size',
        f'Embedding size time, batch={args.base_batch_size} seq len={args.base_seq_len} heads={args.base_heads} attn={args.attn_type} mask={args.mask} fp={args.fp}',
...
do_plot(heads_df.heads, heads_df, 'Heads',
        f'Heads time, batch={args.base_batch_size} seq len={args.base_seq_len} embed size={args.base_embed_size} attn={args.attn_type} mask={args.mask} fp={args.fp}',
